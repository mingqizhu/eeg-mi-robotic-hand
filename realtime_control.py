...
# MODEL_PATH = "best_model.pth" # Selected dynamically
# SCALER_PATH = "scaler_params.npz"
# SERIAL_PORT = "COM5" # Prompted now
...
class VotingFilter:
    def __init__(self, window_size=7, grasp_threshold=4, rest_threshold=6):
    ...
    def update(self, prediction):
        ...
        # New Rule: 5 Consecutive Full Null Windows -> Rest (Harder Safety)
        if len(self.window) > 0 and counts[self.NULL] == len(self.window):
            self.null_streak += 1
            if self.null_streak >= 5:
                return self.REST
        else:
            self.null_streak = 0
        ...
        # Decision Logic
        # Grasp: Easier (4)
        if counts[self.GRASP] >= self.grasp_threshold:
            return self.GRASP
        # Rest: Harder (6)
        elif counts[self.REST] >= self.rest_threshold:
            return self.REST
        else:
...
def get_serial_port():
    ...
    port_input = input(f"Enter Serial Port [{default_port}]: ").strip()
    return port_input.upper() if port_input else default_port



def select_model_and_scaler():
    """
    Lists available best_model_*.pth files and finds matching scaler params.
    Returns (model_path, scaler_path).
    """
    # Find all model files
    model_files = glob.glob("best_model*.pth")
    if not model_files:
        print("No model files found (best_model*.pth).")
        return None, None
    ...
    for i, m_path in enumerate(model_files):
        ...
        # Determine expected scaler name
        if base.startswith("best_model_"):
            # best_model_Finn4_83.pth -> scaler_params_Finn4_83.npz
            suffix = base[len("best_model_"):-4] 
            s_name = f"scaler_params_{suffix}.npz"
        elif base == "best_model.pth":
            s_name = "scaler_params.npz"
        else:
            s_name = "scaler_params.npz"
        ...
        if os.path.exists(s_name):
            ...
            status = "(NO matching scaler, using defaults)"
            s_path = None
            
        print(f"[{i+1}] {base}  {status}")
        valid_choices.append((m_path, s_path))
        
    while True:
        choice = input(f"Enter choice (1-{len(valid_choices)}): ").strip()
        if not choice: continue
        try:
            idx = int(choice) - 1
            if 0 <= idx < len(valid_choices):
                print(f"Selected: {valid_choices[idx][0]}")
...
def main():
    ...
    # 0. Select Model
    selected_model_path, selected_scaler_path = select_model_and_scaler()
    ...
    # Updated to 2.0s to match Data Augmentation training logic
    INPUT_WINDOW_SEC = 2.0
    ...
    try:
        ...
        model.to(device)
        model.eval()
        print(f"[System] Model {selected_model_path} loaded.")
    except Exception as e:
    ...
    # Load Scaler
    if selected_scaler_path and os.path.exists(selected_scaler_path):
        scaler = np.load(selected_scaler_path)
    ...
    voting = VotingFilter(window_size=7, grasp_threshold=4, rest_threshold=6)
    
    target_port = get_serial_port()
    commander = SerialCommander(target_port, BAUD_RATE)
